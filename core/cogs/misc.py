...
class Misc(commands.Cog): 
    ...
    async def history(self, ctx, id=None):
        ...
        data = APIRequest.get_history(id)
        if not data.get('data'):
            return await ctx.send('Oops, something wrong happened...')
    ...
    @commands.command()
    async def mission(self, ctx, id=None):
        """
        Get data from a specific Mission by ID.
        usage example:
            x:mission 9D1B7E0
        """
        data = APIRequest.get_mission(id)
